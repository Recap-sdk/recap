...
extension FamilyLoginViewController {
            ...
            db.collection("users").document(userDocID).collection("family_members")
                ...
                .getDocuments { [weak self] snapshot, error in
                    ...
                    if let error = error {
                        ...
                        self.showAlert(
                            message:
                                "Error checking family member status: \(error.localizedDescription)"
                        )
                        return
                    }
                    ...
                    if let documents = snapshot?.documents, !documents.isEmpty {
                        ...
                        UserDefaults.standard.set(
                            profileImageURL, forKey: Constants.UserDefaultsKeys.familyMemberImageURL
                        )
                        UserDefaults.standard.set(
                            true, forKey: Constants.UserDefaultsKeys.isFamilyMemberLoggedIn)
                        ...
                        self.stopLoadingAnimation(loadingAnimation)
                        self.showFamilyRegistration(
                            email: email, profileImageURL: profileImageURL, userDocID: userDocID)
                    }
                }
        }
    }
    ...
    private func animateSlideToMainScreen() {
        ...
        navigationController.view.frame = CGRect(
            x: window.frame.width, y: 0, width: window.frame.width, height: window.frame.height)
...
extension FamilyLoginViewController: ASAuthorizationControllerDelegate,
    ASAuthorizationControllerPresentationContextProviding
{
    ...
    func authorizationController(
        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            // Get user information (Apple only provides this on FIRST sign-in)
            ...
            // Store name information from Apple - it's only provided during first sign-in
            if !firstName.isEmpty || !lastName.isEmpty {
                ...
                // Store permanently with the user ID for future sign-ins
                ...
                // Try retrieving previously stored name information
                let appleUserDataKey = "appleUser_\(userIdentifier)"
                    ...
                    as? [String: String]
                {
                    ...
                    if !storedFirstName.isEmpty {
                        UserDefaults.standard.set(storedFirstName, forKey: "appleSignInFirstName")
                    }
                    if !storedLastName.isEmpty {
                        UserDefaults.standard.set(storedLastName, forKey: "appleSignInLastName")
                    }

                    print("Retrieved stored Apple user data: \(storedFirstName) \(storedLastName)")
                }
            }
            ...
            Auth.auth().signIn(with: credential) { [weak self] authResult, error in
                ...
                self.checkFamilyMemberStatus(
                    userDocID: userDocID, email: email, profileImageURL: "",
    ...
    private func checkFamilyMemberStatus(
        userDocID: String, email: String, profileImageURL: String,
        loadingAnimation: LottieAnimationView? = nil
    ) {
        // Check if this email is already registered as a family member
        let db = Firestore.firestore()
        db.collection("users").document(userDocID).collection("family_members")
